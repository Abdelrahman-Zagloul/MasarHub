...
namespace MasarHub.Application.UnitTests.Features.Lessons.Commands.DeleteLesson
{
    [Trait("UnitTests.Feature.Lessons", "DeleteLesson")]
    public sealed class DeleteLessonCommandHandlerTests
    {
        ...
        public DeleteLessonCommandHandlerTests()
        {
            ...
            _sut = new DeleteLessonCommandHandler(_unitOfWorkMock.Object, _lessonQueryMock.Object, _lessonRepositoryMock.Object);
        }

        [Fact]
        public async Task Handle_ModuleNotFound_ReturnsNotFoundError()
        {
            var command = new DeleteLessonCommand(Guid.NewGuid(), Guid.NewGuid(), InstructorId);
        ...
        [Fact]
        public async Task Handle_DeleteFails_ReturnsConflictError()
        {
            var lesson = new FakeLesson();
            var command = new DeleteLessonCommand(Guid.NewGuid(), Guid.NewGuid(), InstructorId);
            ...
            result.IsFailure.Should().BeTrue();
            result.Errors.Should().Contain(e => e.Code == "lesson.cannot_delete_published_lesson");
            _unitOfWorkMock.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Never);
        }
        ...
        {
            public FakeLesson() : base(Guid.NewGuid(), false, "Test", 1, null) { }
        }
    }
}
