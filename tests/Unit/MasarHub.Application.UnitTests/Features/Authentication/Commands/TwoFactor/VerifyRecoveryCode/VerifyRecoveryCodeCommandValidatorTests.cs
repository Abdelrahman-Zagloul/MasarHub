using FluentAssertions;
using MasarHub.Application.Features.Authentication.Commands.TwoFactor.VerifyRecoveryCode;

namespace MasarHub.Application.UnitTests.Features.Authentication.Commands.TwoFactor.VerifyRecoveryCode
{
    [Trait("UnitTests.Feature.Auth", "VerifyRecoveryCode")]
    public sealed class VerifyRecoveryCodeCommandValidatorTests
    {
        private readonly VerifyRecoveryCodeCommandValidator _sut = new();

        [Fact]
        public void Validate_ValidCommand_ReturnsNoErrors()
        {
            var command = new VerifyRecoveryCodeCommand(Guid.NewGuid(), "recovery-code-1");

            var result = _sut.Validate(command);

            result.IsValid.Should().BeTrue();
        }

        [Fact]
        public void Validate_InvalidChallengeId_ReturnsValidationError()
        {
            var command = new VerifyRecoveryCodeCommand(Guid.Empty, "recovery-code-1");

            var result = _sut.Validate(command);

            result.IsValid.Should().BeFalse();
            result.Errors.Should().Contain(e => e.PropertyName == "ChallengeId");
        }

        [Theory]
        [InlineData("")]
        [InlineData("   ")]
        public void Validate_InvalidCode_ReturnsValidationError(string code)
        {
            var command = new VerifyRecoveryCodeCommand(Guid.NewGuid(), code);

            var result = _sut.Validate(command);

            result.IsValid.Should().BeFalse();
            result.Errors.Should().Contain(e => e.PropertyName == "Code");
        }
    }
}
