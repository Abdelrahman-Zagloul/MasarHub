...
{
    public sealed class OrderJob : IOrderJob
    {
        private readonly ICourseQuery _courseQuery;
        private readonly IAppEmailService _appEmailService;
        private readonly FrontendURLsSettings _frontendURLsSettings;

        public OrderJob(ICourseQuery courseQuery, IAppEmailService appEmailService, IOptions<FrontendURLsSettings> options)
        {
        ...
        public async Task SendCreatedEmailAsync(Guid userId, string orderNumber, decimal finalAmount, Guid orderId)
        {
        ...
        public async Task SendCancelledEmailAsync(Guid userId, string orderNumber, Guid orderId)
        {
            var userInfo = await _courseQuery.GetUserInfoAsync(userId);
            if (userInfo == null)
                return;
